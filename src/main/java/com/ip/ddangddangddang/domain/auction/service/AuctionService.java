...
public class AuctionService {
    ...
    public void createAuction(AuctionRequestDto requestDto, Long userId) { // Todo fileId 곂칠때 duplicated error
        User user = userService.getUserById(userId)
            .orElseThrow(() -> new UserNotFoundException("회원이 존재하지 않습니다.")); //  없는게 정상 로직일 수 있다 -> 없는게 정상로직일 때 옵셔널로 받아오고 있어야하는 로직은 orelsethrow를 써도 된다. 없을 게 정상로직으로 추가될 수 있으니 확장성 측면에서 옵셔널로 받는게 좋다
        File file = fileService.getFileById(requestDto.getFileId())
            .orElseThrow(() -> new FileNotFoundException("없는 이미지 입니다."));
    ...
    public List<AuctionListResponseDto> getAuctions(Long userId, StatusEnum status, String title) {
        User user = userService.getUserById(userId)
        ...
        return auctionRepository.findAllByFilters(townList, status, title).stream()
            ...
            auction -> new AuctionListResponseDto(
                ...
                auction.getPrice())).collect(Collectors.toList());
    }
